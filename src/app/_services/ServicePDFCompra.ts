...
export class PdfGenerationServiceCompra {
	...
	async LoadDataServiceOrder(corden: any) {
		await this.http.get(environment.apiUrl + `/api/v1/events/getServiceOrder/${corden}`).subscribe(async (response: any) => {
			if (response.status) {
				...
				this.ccotizacion = response.data.ccotizacion;
				...
				await this.http.post(environment.apiUrl + `/api/v1/events/getQuoteById/${this.ccotizacion}`, {}).subscribe(async (response: any) => {
					this.cotizacion = await response.data[0]
					this.montototal = await this.cotizacion.mmontototal + ' ' + this.cotizacion.xmoneda
					console.log(this.montototal);
					await this.http.post(environment.apiUrl + `/api/v1/events/quotes-replacement/detail/${this.ccotizacion}`, {}).subscribe(async (response: any) => {
						...
						this.bodyReplacements.push([
							{ text: 'Cantidad', style: 'tableHeader' }, 
							{ text: 'Repuesto', style: 'tableHeader' }, 
							{ text: 'Monto Unitario', style: 'tableHeader' }, 
							{ text: 'Subtotal', style: 'tableHeader' }, 
							{ text: 'IVA', style: 'tableHeader' },
							{ text: 'Monto Total', style: 'tableHeader' }
						],)
						for (const replacement of this.quotesFinallyAceptedList) {
							let ivaReplacement = 0
							ivaReplacement = (replacement.mtotalrepuesto * 0.16)
							this.bodyReplacements.push(
								[
									replacement.ncantidad, 
									replacement.xrepuesto,
									replacement.munitariorepuesto + ' ' + replacement.xmoneda,
									replacement.mtotalrepuesto + ' ' + replacement.xmoneda,
									ivaReplacement + ' ' + replacement.xmoneda,
									replacement.mivaindividual + ' ' + replacement.xmoneda									
								],
							)
						}
				...
				content: [
					...
					},
					{
						...
						table: {
							widths: ['*'],
							body: [
								[{ text: `PIEZAS AFECTADAS:`, alignment: 'start', fillColor: '#fff', bold: true, border: [false, false, false, false] }],
							]
						}
					},
					{
						...
						margin: [0, 15, 0, 0],
						table: {
							widths: [ 'auto', 175, '*', '*', '*' , '*' ], // Ajusta los anchos de las columnas según tus necesidades
							headerRows: 1,
							body: this.bodyReplacements
						},
					...
					},
					{
						...
						table: {
							widths: ['*'],
							body: [
								[{ text: `DETALLES DEL SERVICIO:`, fillColor: '#fff', bold: true, border: [false, false, false, false] }],
							]
						}
					},
					{
						style: 'tableExample',
						table: {
							widths: [ 425, '*' ], // Ajusta los anchos de las columnas según tus necesidades
							headerRows: 1,
							body: [
								[
									{ text: 'Servicio', style: 'tableHeader' }, 
									{ text: 'Total Servicio', style: 'tableHeader' }
								],
								[
									this.xservicio,
									this.montototal
